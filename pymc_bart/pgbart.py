...
class PGBART(ArrayStepShared):
    ...
    def __init__(
        ...
        **kwargs
        ...
        init_mean = self.bart.initial_mean
    ...
    def astep(self, _):
        ...
        #    print(name,shared_var.get_value())
    ...
    def update_weight(self, particle: ParticleTree, odim: int) -> None:
        ...
        delta = (
            np.identity(self.trees_shape)[odim][:, None, None, None] * particle.tree._predict()[None, :, :, :]
        )
...
def draw_leaf_value(
    ...
    if y_mu_pred.size == 0:
        return np.zeros(shape), np.zeros(shape), linear_params
